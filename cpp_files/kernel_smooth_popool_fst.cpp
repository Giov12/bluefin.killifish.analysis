...
struct Chrom {
    ...
    vector<float> r1_vs_y1;
    vector<float> r1_vs_y2;
    vector<float> r2_vs_y1;
    vector<float> r2_vs_y2;
    vector<float> smoothed_1;
    vector<float> smoothed_2;
    vector<float> smoothed_3;
    vector<float> smoothed_4;
...
float
parse_field(const string &field){

    uint i = field.find_first_of('=');

    return std::stof(field.substr(i + 1));
...
parse_line(Entry &e, const string &line){
    ...
    */
    ...
    e.r1_vs_y1 = parse_field(parts[6]);
    e.r1_vs_y2 = parse_field(parts[7]);
    e.r2_vs_y1 = parse_field(parts[8]);
    e.r2_vs_y2 = parse_field(parts[9]);
...
void 
smooth_fsts(Chrom &current_chrom, fstream &filestream){
    ...
    */
    ...
    float r1_vs_y1, r1_vs_y2, r2_vs_y1, r2_vs_y2;
    ...
    for (int site = 0; site < n_sites; site++){
        ...
        for (int pos = start_pos; pos < end_pos; pos++){
            n_window_sites++;
            r1_vs_y1 = current_chrom.r1_vs_y1[pos]; // get fsts
            r1_vs_y2 = current_chrom.r1_vs_y2[pos];
            r2_vs_y1 = current_chrom.r2_vs_y1[pos];
            r2_vs_y2 = current_chrom.r2_vs_y2[pos];
            ...
            avg1 += (r1_vs_y1 * weight); // accumulate sum
            avg2 += (r1_vs_y2 * weight); // accumulate sum
            avg3 += (r2_vs_y1 * weight); // accumulate sum
            avg4 += (r2_vs_y2 * weight); // accumulate sum
            window_weight += weight; // accumulate weight
        }
        if (n_window_sites == 0){
            exit(EXIT_FAILURE);
        }
        current_chrom.smoothed_1.push_back((float)(avg1/window_weight));
        current_chrom.smoothed_2.push_back((float)(avg2/window_weight));
        current_chrom.smoothed_3.push_back((float)(avg3/window_weight));
        current_chrom.smoothed_4.push_back((float)(avg4/window_weight));
    }

    // now to write to output
    for (int i = 0; i < n_sites; i++){
        filestream << current_chrom.name          << '\t' << current_chrom.positions[i] << '\t' 
                   << current_chrom.smoothed_1[i] << '\t' << current_chrom.smoothed_2[i] << '\t'
                   << current_chrom.smoothed_3[i] << '\t' << current_chrom.smoothed_4[i] << '\n';
    }
...
update_chrom(Chrom &current_chrom, Entry &e){
    ...
    current_chrom.r1_vs_y1.clear();
    current_chrom.r1_vs_y2.clear();
    current_chrom.r2_vs_y1.clear();
    current_chrom.r2_vs_y2.clear();
    current_chrom.smoothed_1.clear();
    current_chrom.smoothed_2.clear();
    current_chrom.smoothed_3.clear();
    current_chrom.smoothed_4.clear();

    // update
    current_chrom.name = e.chrom;
    current_chrom.positions.push_back(e.position);
    current_chrom.r1_vs_y1.push_back(e.r1_vs_y1);
    current_chrom.r1_vs_y2.push_back(e.r1_vs_y2);
    current_chrom.r2_vs_y1.push_back(e.r2_vs_y1);
    current_chrom.r2_vs_y2.push_back(e.r2_vs_y2);
    ...
    do
    {
        ...
        else
        {
            ...
            if (current_chrom.name == e.chrom){
                ...
                smooth_fsts(current_chrom, file_stream);
...
int main(int argc, char *args[])
{
    ...
    message = "./ kernel_smooth_popool_fst -f fst_file [required]\n";
