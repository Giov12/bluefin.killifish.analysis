...
void parse_compressed(const char *depth_file, int subsample){
    ...
    vector<int> depths;
    vector<int> temp; // hold temp depths
    vector<int> temp2; // hold temp pos
    ...
    if (temp.size() > 0){
        median_depth = median(temp);
        depths.push_back(median_depth);
        positions.push_back(temp2.back()); // last called position
