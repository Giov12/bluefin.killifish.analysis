...
def get_gene_name(gene_info: str) -> str:

    fields = gene_info.split(';')

    if (len(fields) == 1):
        if ("gene_id" not in fields[0]):
...
def get_y_spacing(svg_height, y_offset, max_depth):
    """adjust svg height for # of seqs"""
...
def draw_y_axis(cairo_context, x_pos, svg_height, y_offset, spacing, ceiling, max_cmh):
...
def plot_depth(cairo_context, sites, svg_height, y_offset, x_start, allocated_pos, allocated_pos_y, leftbound, ceiling, color):
...
def draw_image(exons, leftbound, rightbound, all_sites, gaps, chrom, max_depth):
    ...
    spacing, ceiling, allocated_space_y = get_y_spacing(svg_height, y_offset, max_depth)
    seq_len = rightbound - leftbound

    ims = cairo.SVGSurface(f"{chrom}-{leftbound}-{rightbound}_depth.svg", svg_width, svg_height)
    ...
    for i, sites in enumerate(all_sites):
        ...
        plot_depth(cairo_context, sites, svg_height, y_offset, chrom_x_pos, allocated_pos, allocated_space_y, leftbound, ceiling, color)

    draw_y_axis(cairo_context, y_axix_x_pos, svg_height, y_offset, spacing, ceiling, max_depth)
